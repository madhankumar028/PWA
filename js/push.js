(function (window) {
  ...
  var pushNotificationElement = document.querySelector('#send-notification');
  ...
  function subscribePush() {
    navigator.serviceWorker.ready.then(function(registration) {
      ...
      registration.pushManager.subscribe({
        ...
        applicationServerKey: 'BNhTttYLYJxkxjWsgh-W43cD822_nt_HORc0DDTrETbkNGGVZB6SAjzjJ3xX4ArdimnVinql_mC8_k0_BmIsXdU'
      })
      .then(function (subscription) {
  ...
  function triggerNotification(subscription) {
    fetch('/send__notification', {
      method: 'post',
      headers: {
  ...
  pushNotificationElement.addEventListener('click', function () {
    const subscription = JSON.parse(localStorage.getItem('subscriptionObject'));
    var isSubscribed = (fabPushElement.dataset.checked === 'true');

    if (isSubscribed && Object.keys(subscription).length) {
      triggerNotification(JSON.parse(localStorage.getItem('subscriptionObject')));
      return;
    }

    toast('Subscribe first to trigger notification');
  });
